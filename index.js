...
RaspberryPiInfo.prototype.setUpServices = function () {
	...
	this.fakeGatoHistoryService = new FakeGatoHistoryService("weather", this, { storage: 'fs', minutes: 5, size: 8064 });

	let uuid1 = UUIDGen.generate(that.name + '-Uptime');
	info = class extends Characteristic {
	...
	load = class extends Characteristic {
		constructor() {
			super('Average Load', uuid2);
			this.setProps({
				format: 'string',
				perms: ['pr', 'ev']
			});
